d3.biHiSankey = function () {
  ...
  function computeNodeValues() {
    nodes.forEach(function (node) {
      ...
      node.netFlow = d3.sum(visible(node.targetLinks), value); //d3.sum(visible(node.targetLinks), value) - d3.sum(visible(node.sourceLinks), value);
  ...
  function computeNodeXPositions() {
    var remainingNodes = nodes,
        ...
        setValues = function (node) {
          ...
          //node.sourceLinks.forEach(addToNextNodes, node); //Mantém tudo na mesma linha
        };
      ...
      nodesByXPosition.forEach(function (nodes) {
          ...
          for (i = 0; i < n; ++i) {
              ...
              if (node.state != "contained") {
                  ...
                  y0 = node.y + node.heightAllowance + nodeSpacing + (node.children.length * nodeSpacing);
              }
          }
